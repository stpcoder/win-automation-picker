...
| 파일 | 용도 |
| --- | --- |
| [AEWorkbench-Windows-x64.zip](https://github.com/stpcoder/win-automation-picker/releases/latest/download/AEWorkbench-Windows-x64.zip) | EXE 5개와 SHA-256 manifest가 들어 있는 권장 묶음 |
...
`main` push 시 GitHub Actions가 실제 QDL/Genio parser contract를 검증한 뒤 다섯 Windows
실행 파일을 release job에서 다시 빌드하고 `latest` asset을 직접 갱신합니다. Actions의 만료형
artifact 저장 공간을 배포 경로로 사용하지 않습니다. QDL/Genio 실행 파일 자체는 묶지 않으며
공식 저장소, 검증한 source commit/version, parser dry-run 범위는
`windows-release-manifest.json`에 기록합니다. 문서 변경은 `gh-pages`에 배포됩니다.
