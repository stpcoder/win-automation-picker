...
- 다음 외부 클릭 아래의 Windows UIA component를 캡처합니다.
...
2. 상단 `클릭 녹화`를 누르고 대상 프로그램의 버튼을 클릭합니다.
3. 작업실에 생긴 블록의 이름을 오른쪽에서 `시작 클릭`처럼 바꾸고 Enter를 누릅니다.
4. 입력이 필요하면 상단 `입력값`을 채우고 `입력 녹화`로 입력칸을 잡습니다.
5. 왼쪽 팔레트에서 `N번 반복` 또는 `만약 ...` 블록을 끌어 놓습니다.
6. 실행 블록을 C자형 컨테이너 안으로 끌어 넣습니다.
7. 블록을 위아래로 끌어 순서를 조정합니다.
8. 상단 `실행`으로 시험하거나 `Python 내보내기`로 저장합니다.

녹화는 지속적인 백그라운드 기록이 아닙니다. `클릭 녹화`와 `입력 녹화`는 각각 다음 외부 클릭 한 번만 캡처한 뒤 종료됩니다. 입력 녹화 시에는 실제 문자를 입력하지 않으며, 실행 시점에 입력합니다.
...
2. `Connection Setup`에 FTP 계정과 root directory를 입력합니다.
3. `Init folders`로 전용 폴더만 생성합니다.
4. slave별 `.info`를 내보내 각 PC의 exe 옆에 둡니다.
5. slave에서 `This PC Agent > Start agent`를 실행합니다.
6. master에서 매크로를 업로드하고 대상 slave에 제출합니다.

FTP 연결은 전송 시점에만 열고 닫으며, poll jitter, screenshot 최소 간격, 결과 보관 개수로 부하를 제한합니다. 기존 FTP의 다른 폴더는 건드리지 않고 설정한 root 아래만 사용합니다.
