...
`Field Verified: False`는 패키지 오류가 아니라 실제 장비 성공 자료와 아직 대조되지 않았다는 의미입니다. 구조 검사는 통과했지만 실제 protocol 호환성을 보증하지 않습니다.

## 3. 한 PC에 4개 실장기 배정

1. `[SEQ]` 패키지를 선택합니다.
2. 빠른 실행의 `SK Commander 런처`에서 `[FLOW]` 런처를 고릅니다.
3. `PC / 슬롯 / CH별 실행표 > 설정 PC 불러오기`를 누릅니다.
4. PC 행을 선택하고 `행 복제`를 세 번 눌러 네 행을 만듭니다.
5. 각 셀을 더블클릭해 값을 설정합니다.
