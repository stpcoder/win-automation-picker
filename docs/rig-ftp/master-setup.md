...
2. `PC / 슬롯 / CH별 실행표 > 설정 PC 불러오기`를 누릅니다.
...
한 PC에 SK Commander가 4개라면 해당 PC 행을 선택하고 `행 복제`로 네 행을 만든 뒤 각 행의 `CH`, `슬롯`, `SEQ / 매크로`, `SK Commander 런처`를 지정합니다. 자세한 예시는 [SEQ Generator와 SK Commander 연동](seq-integration.md)을 확인합니다.
