...
![CH9~CH12의 Grid 진행과 판정을 자연 순서로 표시한 AE 캠페인 보드](../assets/screenshots/04-campaign-monitoring.png)
...
1. `2 자동화 준비`에서 검증된 `.rigseq.zip`을 `서버 라이브러리 등록`으로 올립니다.
2. `1 오늘 작업 > 캠페인`으로 이동합니다.
...
1. 캠페인 화면에서 `실행 준비`를 누릅니다.
2. 해당 `[SEQ]` 패키지를 선택합니다.
3. SK Commander launcher `[FLOW]`를 선택합니다.
4. `Rig 대상 불러오기`를 누릅니다.
...
1. `PC · CH 상태`에서 대상 PC를 선택하고 `새로고침`을 누릅니다.
2. 실패 행을 선택합니다.
