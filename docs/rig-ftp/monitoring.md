...
![PC heartbeat와 PASS/FAIL 결과를 한 화면에서 보는 상태 모니터링](../assets/screenshots/05-pc-channel-monitoring.png)
...
화면 component의 텍스트/색상만 새로 판정하려면 `오늘 작업 > 실행 > 운영 도구 열기`에서
workflow와 대상 PC를 지정한 뒤 `상태 규칙 1회`를 누릅니다. 이 job은 클릭, 입력, 키
...
보관 개수는 `3 Rig 설정 > Master · 원격 PC > 고급 정책`에서 조정합니다.

## Emergency stop

1. `1 오늘 작업 > 실행`으로 이동합니다.
2. 상단 `긴급 중단`을 누릅니다.
3. 확인창에서 승인합니다.

실행 중인 작업 하나만 정확히 중단하려면 상태표에서 PC를 선택하고
`더보기 > 선택 작업 긴급 중단`을 사용합니다. 전체 stop signal 해제는 해당 Slave의
`3 Rig 설정 > 이 PC Agent > 더보기`에서 수행합니다.
