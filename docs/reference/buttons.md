...
| `값 편집` | 매크로가 찾은 PC별 변수를 표에서 입력하고 작업 파일에 저장 |
| `시험` / `중지` | 이 PC에서 실제 매크로를 실행하거나 정상 중단 |
| `Python 내보내기` | 현재 Scratch source를 slave 내장 엔진용 FLOW로 export |
| `버튼 추가` | 현재 매크로를 이름 붙인 빠른 선택 버튼으로 등록 |
| `이름 / 메모` | 선택 버튼의 표시 이름과 설명 수정 |
| `위치 <` / `위치 >` | 선택 버튼의 순서 변경 |
...
| `위로` / `아래로` | 같은 컨테이너 안에서 선택 블록 이동 |
| `앞 블록 안으로` | 선택 블록을 바로 앞 컨테이너 안으로 이동 |
| `컨테이너 밖으로` | 선택 블록을 부모 컨테이너 다음으로 이동 |
