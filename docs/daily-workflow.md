...
| Rig PC를 추가하거나 바꿀 때 | `3 Rig 설정` | 4계층 구성 검사, PC/실장기 inventory, Slave Agent 준비 완료 |
...
1. `3 Rig 설정 > Master · 원격 PC > 연결 구조`에서 4계층을 검사한 뒤 `Master · FTP`를 엽니다.
...
1. `실장기 연결 PC`를 엽니다.
2. `연결 PC 추가`로 `PC04`, `rig-pc-04`, 자산 ID, Windows 이름, IP, 위치와 PC별 값을 등록합니다.
3. PC를 선택하고 `실장기 관리`에서 물리 실장기 ID/Serial/위치, `CH9~CH12`, Slot, COM/HWID, SoC, Binary, 자재를 등록합니다.
...
현장에서 SK Commander의 Start를 직접 누르는 PC는 읽기 전용 monitor workflow를
`현장 SK 감시`에서 선택하고 `감시 시작`을 누릅니다. 상태 변화만 결과 이력으로 남고,
Master 보드에는 시작 위치가 `현장 PC`로 표시됩니다.
...
3. `CH / 자재 / Binary`에서 운용 방식/시작 위치, Grid, 자재, Binary와 각 CH 상태를 확인합니다.
4. 실패 행은 결과표에서 더블클릭해 로그를 봅니다.
5. 직접 COM 원본이 필요하면 `더보기 > 선택 결과 증거 ZIP 저장`을 사용합니다.
6. 화면이 필요할 때만 `전체 화면 보기`를 누릅니다.
7. 즉시 멈춰야 하면 오늘 실행 화면의 `긴급 중단`을 사용합니다.
8. 원인을 수정한 뒤 오늘 실행표에서 실패 행만 켜고 다시 `실행 시작`을 누릅니다.
