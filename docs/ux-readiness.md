...
8. 현재 workflow를 FTP에 올립니다. 이 패키지는 `내장 워크플로 엔진`으로 표시됩니다.
9. master의 `PC별 매크로 실행표`에서 PC마다 Seq와 기타 값을 다르게 입력한 뒤 전송합니다.
10. `상태 모니터링`에서 상태, 현재 단계, 결과 이력, 탭별 모니터 보드와 요청한 전체 화면을 확인합니다.
