...
| SEQ Generator 연동 | 80% | 버전 bundle, 구조 검사, SHA-256 재검사, PC/슬롯/CH 배정, SK Commander 런처 지원 |
| 원격 세션 모니터링 | 85% | online/offline, 실행 단계, 결과 표, 탭별 모니터 보드, 화면 요청, Excel 지원 |
| 초기 설정 UX | 80% | 연결 확인, PC/변수 표 편집, PC별 `.info` 생성 지원 |

## 권장 사용자 흐름

1. `RigFtpCommander > 연결 설정`에서 FTP 주소를 입력하고 `연결 확인`을 누릅니다.
2. `공통 변수`와 `Slave PC 목록`을 표에서 추가합니다. CH가 없는 PC도 가능하며 변수 이름은 자유입니다.
3. `실행 및 배포 > 서버 폴더 초기화`로 전용 root 아래 폴더를 만듭니다.
4. `Slave .info 내보내기`로 PC별 설정을 만들고 각 PC에 `RigFtpCommander.exe`와 함께 둡니다.
5. 각 slave에서 `이 PC Agent > Agent 시작`을 누릅니다.
6. `WinAutomationPicker > 연속 녹화 시작` 후 실제 프로그램에서 클릭과 입력을 수행하고 녹화를 정지합니다.
7. 입력 블록을 고정값 또는 `${변수}`로 정리하고 반복/조건/모니터 블록을 drag/drop 합니다.
8. 현재 workflow를 FTP에 올립니다. 이 패키지는 `[FLOW]`와 `내장 워크플로 엔진`으로 표시됩니다.
9. SEQ 업무라면 Test Sequence Generator에서 `Validate > Export Rig Package` 후 `.rigseq.zip`도 올립니다.
10. master의 `PC / 슬롯 / CH별 실행표`에서 PC마다 SEQ, 런처, CH와 기타 값을 입력한 뒤 전송합니다.
11. `상태 모니터링`에서 상태, 현재 단계, 결과 이력, 탭별 모니터 보드와 요청한 전체 화면을 확인합니다.
...
- 현재 `.rigseq.zip`의 호환성 등급은 `structural`입니다. 실제 성공 SEQ, SK Commander log, raw serial log가 없으므로 command timing과 장비 결과까지 field verified된 상태는 아닙니다.
