...
class MacroShortcut:
    ...
    def from_mapping(cls, data: dict[str, Any]) -> "MacroShortcut":
        ...
        if not name or not project_path:
            raise ValueError("자동 실행 버튼에는 이름과 순서 파일이 모두 필요합니다.")
        return cls(
            name=name,
...
class AEWorkbenchProject:
    ...
    def move_shortcut(self, name: str, delta: int) -> "AEWorkbenchProject":
        ...
        try:
            ...
            raise ValueError(f"자동 실행 버튼을 찾을 수 없습니다: {name}") from exc
...
def inspect_automation_project(path: str | Path) -> MacroProjectInspection:
    ...
    if not step_count:
        issues.insert(0, "자동 실행 순서에 실행할 블록이 없습니다.")
    ...
    *,
    ...
    if recipe_path:
        ...
        if not recipe_matches:
            issues.append("SEQ 작성 내용이 변경되어 오류 검사와 내보내기를 다시 해야 합니다.")
...
def assess_workbench(
    ...
    *,
    ...
    macro_ready = False
    sequence_message = "SEQ 작성 파일과 검사 완료 SEQ를 선택하세요."
    macro_message = "자동 실행 순서 파일과 Python 실행 파일을 준비하세요."
    ...
    if project.macro_project_path:
        try:
            ...
            macro_message = f"자동 실행 순서 확인 실패: {exc}"
            ...
            if macro.ok and not export_current:
                issues.append(
                    "Python 실행 파일이 없거나 현재 자동 실행 순서보다 오래되었습니다."
                )
    else:
        issues.append(macro_message)
